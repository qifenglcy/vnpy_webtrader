...
vnpyRouter=APIRouter()
app=vnpyRouter


@app.post("/login", summary="登录", response_model=Token)
async def user_auth(form: UserLogin):
    ...
    if user_obj :
        if verify_password(form.password, user_obj.password):
            ...
            return {"id":user_obj.id, "access_token": access_token, "token_type": "bearer"}
        return "账号或密码错误"
    return "账号被禁用"
...
def rpc_callback(topic: str, data: Any):
    ...
    #     return
    global cta_log_data
    global event_cta_stoporder
    global event_indicator
    data = {
        "topic": topic,
        "data": to_dict(data)
    }
    # if topic == 'eCtaLog' or topic =='eCtaStrategy' or topic =='eLog':print('rpc_callbackrpc_callback',data)
    if not topic:return
    if topic == 'eCtaLog':
        cta_log_data.append(data['data'])
    elif topic =='eCtaStopOrder':
        for i in event_cta_stoporder:
            if data['data']['stop_orderid'] == i['stop_orderid']:
                event_cta_stoporder.remove(i)
        event_cta_stoporder.append(data['data'])
    elif topic =='eIndicationData':
        event_indicator.append(data['data'])
        if len(event_indicator)>10000:
...
def startup_event() -> None:
    """应用启动事件"""
    global rpc_client
    if rpc_client:return rpc_client
