...
class BacktestingEngine:
    ...
    # 在已有成交记录的基础上 统计
    def calculate_result_with_trade(self):

        record = self.allTrade
        record = sorted(record, key=lambda record: record['Date'], reverse=False)
        ...
        for item in record:
            d = item['Date'].split(' ')[0] + ' '+item['Date'].split(' ')[1]
        ...
        for item in record:
            ...
            if item['Side'] == 'open buy':
                _direction = Direction.LONG
                _offset = Offset.OPEN
            else: 
                _direction = Direction.SHORT
                _offset = Offset.CLOSE

            d = item['Date'].split(' ')[0] + ' '+item['Date'].split(' ')[1]
            price = float(item['Price'])

            self.daily_results[d].add_trade(
                TradeData(
                    gateway_name='BACKTESTING', 
                    ...
                    offset=_offset, 
                    price= price, 
                    volume= float(item['Amount'])/price, 
                    datetime= item['Date'].split(' ')[0] + ' '+item['Date'].split(' ')[1]
                )
            )
        ...
        for daily_result in self.daily_results.values():
            ...
            show_profit+=daily_result.total_pnl
            temp.append({
            ...
            # print('holding_pnl:', daily_result.holding_pnl)
            # print('trading_pnl:', daily_result.trading_pnl)
            # print('total_pnl:', daily_result.total_pnl)
            # print('@@@@')
            # print('图表:',show_profit)
            # print('------------------------------------')
    ...
    # 获取deribit成交记录
    def get_trade_record(self,pdata):
